...
class StoreRequest extends Request
{
    ...
    public function rules()
    {
        return [
            ...
            'shipCompanyId' => ['required', 'numeric', new ExistsCheckRule(ShipCompany::class, '船会社ID')],
            ...
            'scheduleItems.*.polPortId' => ['required', 'numeric', new ExistsCheckRule(Port::class, '積港ID', ['type' => PortType::POL->value])],
            ...
            'scheduleItems.*.podPortId' => ['required', 'numeric', new ExistsCheckRule(Port::class, '揚港ID', ['type' => PortType::POD->value])],
            'scheduleItems.*.podArrivalAt' => ['nullable', 'date'],
            'scheduleItems.*.podArrivedAt' => ['nullable', 'date'],
            'scheduleItems.*.documentCutAt' => ['nullable', 'date'],
            'scheduleItems.*.documentAmCut' => ['nullable', 'boolean'],
            'scheduleItems.*.cyOpenAt' => ['nullable', 'date'],
            'scheduleItems.*.cyCutAt' => ['nullable', 'date'],
            'scheduleItems.*.cyAmCut' => ['nullable', 'boolean'],
    ...
    public function bodyParameters(): array
    {
        return [
            ...
            'scheduleItems.*.cyOpenAt' => [
                'description' => 'コンテナヤードオープン日時',
                'example' => '2023-07-12 15:00',
            ],
            'scheduleItems.*.cyCutAt' => [
                'description' => 'コンテナヤードカット日時',
                'example' => '2023-07-12 15:00',
            ],
            'scheduleItems.*.cyAmCut' => [
                'description' => 'コンテナヤードAMカットフラグ',
                'example' => false,
            ],
