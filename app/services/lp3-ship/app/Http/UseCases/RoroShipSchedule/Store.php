...
class Store
{
    ...
    public function __invoke(array $reqData): ShipSchedule
    {
        ...
        $exists = (new RoroScheduleCheckUnique)($uniCheckData);
        if (!$exists) {
            throw new ShipScheduleSaveException('ユニークチェックで、作成できません', 500);
        }
        ...
        $shipSchedule->type = ShipScheduleType::RORO_SHIP->value;
        ...
        DB::transaction(function () use ($shipSchedule, $reqData) {
            ...
            foreach ($reqData['scheduleItems'] as $val) {
                ...
                if (isset($val['goDownAt']))
                    $scheduleItem->go_down_at = $val['goDownAt'];

                if (isset($val['goDownZipCode']))
                    $scheduleItem->go_down_zip_code = $val['goDownZipCode'];

                if (isset($val['goDownAddress']))
                    $scheduleItem->go_down_address = $val['goDownAddress'];
