...
it('validation will be failed', function ($data) {
    ...
    switch ($type) {
        case 1:
            $res = $this->json('post', 'api/lp3-ship/ship-schedule/cont', $reqData);
        ...
        case 2:
            ...
            $res->assertJsonValidationErrors([
                ...
                'scheduleItems.0.cyAmCut', 'scheduleItems.0.irregularCyCut', 'scheduleItems.0.available'
            ]);
            break;
        case 3:
            $res = $this->json('post', 'api/lp3-ship/ship-schedule/cont', $reqData);
...
it('creates a ship-schedule', function () {
    /** @var TestCase $this */
    ...
    // api call
    $result = $this->json('post', 'api/lp3-ship/ship-schedule/cont', $data);
    ...
    $resultData = Arr::except($resultData, ['id', 'createdBy', 'updatedBy', 'deletedBy', 'createdAt', 'updatedAt', 'deletedAt', 'ship', 'shipCompany', 'type']);

    foreach ($resultData['scheduleItems'] as &$val) {
    ...
    $this->assertEquals(Arr::sort($resultData), Arr::sort($data));
});
