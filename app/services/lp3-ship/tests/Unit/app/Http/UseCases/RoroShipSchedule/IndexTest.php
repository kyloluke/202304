...
it('gets all roro-ship-schedules', function () {
    /** @var TestCase $this */


    $roroShip = Ship::factory()->create(['type' => ShipType::RORO_SHIP->value]);

    $shipSchedules = ShipSchedule::factory()->has(ShipScheduleItem::factory()->count(3), 'scheduleItems')->count(2)->create(['ship_id' => $roroShip->id]);
